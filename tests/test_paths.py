...
from hermes_fry_cards import config as config_mod
from hermes_fry_cards import patcher as patcher_mod
from hermes_fry_cards.config import hermes_home
...
def test_hermes_python_found_via_code_roots(
    ...
    monkeypatch.setattr("hermes_fry_cards.patcher.shutil.which", lambda _: None)
    # 屏蔽系统级 root-mode 路径，避免 CI 机器命中真实安装
    monkeypatch.setattr("hermes_fry_cards.patcher._code_roots",
...
def test_python_from_hermes_cli_unix(
    ...
    monkeypatch.setattr("hermes_fry_cards.patcher.shutil.which", lambda _: str(cli_script))
