...
> 项目更名：hermes-fry-cards → **hermes-fry-cards（🍟 薯条卡片）**，版本号从 0.1.0 重新开始。
...
- 项目改名：hermes-fry-cards → hermes-fry-cards 🍟 薯条卡片
- Python 包名保持 `hermes_fry_cards` 不变（兼容已安装用户）
...
- Merge streaming tool calls into a shared panel (update pending panel instead of creating new ones)
...
- Project renamed: hermes-fry-cards → hermes-fry-cards 🍟
- Python package name unchanged (`hermes_fry_cards`) for backward compat
- Footer disabled in merged panel mode to avoid duplication
- Header style: `🥔 Agent · N轮 思维 · N步 工具 🍟 token`

### Docs

- README restructured: feature table, config table, badges
- INSTALL.md: manual + AI Agent dual-path installation
- English README.en.md synced
